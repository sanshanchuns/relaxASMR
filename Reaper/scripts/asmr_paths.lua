...
local function project_with_config(dir)
...
function M.project_root()
  ...
  if proj and proj ~= "" then
    local found = project_with_config(proj)
    if found then return found end
    -- RECORD_PATH "Audio Files" 时 GetProjectPath 常返回 .../MVI_6918/Audio Files
    local parent = proj:match("^(.+)[\\/]Audio Files$")
    if parent then
      found = project_with_config(parent)
      if found then return found end
    end
  end
...
function M.load_asmr_config()
  ...
  local cfg_path = scripts_dir .. M.sep() .. "asmr_config.lua"
