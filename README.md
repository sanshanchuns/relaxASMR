...
- **人工干预**: 在工程中加载上一步生成的 VST `.rain` 预设，搭配其他素材生成对应时长的环境音效混音，并执行工程的 Render 导出，例如导出成 `MVI_XXXX_3h.wav` 置于 `output/`。

### 4. 合成视频 (导出 MP4)
- **操作**: 在 GUI 点击【开始合成 (export_mp4)】。
- **效果**: 脚本会自动定位该场景 `output/` 文件夹内刚刚生成的混音文件，并将其与原始的 Loop 视频进行合并计算。根据视频宽度的不同，将智能打上 `_4k` 或 `_fhd` 等后缀输出。一切在后台运行，并在日志输出框报告进度，不会卡死主界面。
