...
import logging
from scripts.rain_vst.analyze import VSTParams, DISTANT_NAMES, SPACE_NAMES, CLOSE_NAMES
...
def analyze_and_map_with_clip(image_path: str, on_progress=None):
    ...
    try:
        from scripts.rain_vst.presets_db import OFFICIAL_PRESETS
    except ImportError:
        OFFICIAL_PRESETS = {}

    # K-Nearest Neighbor (KNN) Alignment with Official Presets
    best_preset_name = "None"
    best_preset = {}
    min_dist = float('inf')
    
    for name, op in OFFICIAL_PRESETS.items():
        dist = 0.0
        # Layer penalties (mismatch means it's a different acoustic space)
        if op.get('l1') != cls_key: dist += 2.0
        if op.get('l2') != spc_key: dist += 1.0
        if op.get('l3') != dist_key: dist += 1.0
        
        # Rain intensity distance (continuous)
        dist += abs(op.get('rfdn', 0.5) - rfdn) * 2.0
        dist += abs(op.get('rfin', 0.5) - rfin) * 2.0
    ...
    if on_progress:
        on_progress(f"调音大师对齐: 匹配到官方预设 [{best_preset_name}] (相似度距离: {min_dist:.2f})")
            
    # Fallback baseline if DB missing
    bgwi = best_preset.get('bgwi', 0.8)
    spwi = best_preset.get('spwi', 0.6)
    suwi = best_preset.get('suwi', 0.6)
    sucd = best_preset.get('sucd', sucd)  # fallback to AI inferred sucd
    spbl = best_preset.get('spbl', 0.5)
    glhp = best_preset.get('glhp', 0.0)
    gllp = best_preset.get('gllp', 0.0)
    
    bgeq1x = best_preset.get('bgeq1x', 0.52); bgeq1y = best_preset.get('bgeq1y', 0.45)
    bgeq2x = best_preset.get('bgeq2x', 0.52); bgeq2y = best_preset.get('bgeq2y', 0.52)
    bgeq3x = best_preset.get('bgeq3x', 0.58); bgeq3y = best_preset.get('bgeq3y', 0.52)
    
    eq1x = best_preset.get('eq1x', 0.65); eq1y = best_preset.get('eq1y', 0.52)
    eq2x = best_preset.get('eq2x', 0.55); eq2y = best_preset.get('eq2y', 0.52)
    eq3x = best_preset.get('eq3x', 0.55); eq3y = best_preset.get('eq3y', 0.55)
        
    # NEW MAPPING: l1=Close, l2=Space, l3=Distant
    p = VSTParams(
        scene_cn=scene_cn,
        scene_en=scene_en,
        l1=cls_key, l1_name=_layer_label(cls_key, CLOSE_NAMES),
        l2=spc_key, l2_name=_layer_label(spc_key, SPACE_NAMES),
        l3=dist_key, l3_name=_layer_label(dist_key, DISTANT_NAMES),
        adv=1,
        bgwi=bgwi, bgpn=0.5, bgeq1x=bgeq1x, bgeq1y=bgeq1y, bgeq2x=bgeq2x, bgeq2y=bgeq2y, bgeq3x=bgeq3x, bgeq3y=bgeq3y,
        spbl=spbl, spwi=spwi, sppn=0.5,
        sucd=sucd, suwi=suwi, supn=0.5,
        rfdn=rfdn, rfin=rfin, rffc=rffc, rfch=0.4,
        eq1x=eq1x, eq1y=eq1y, eq2x=eq2x, eq2y=eq2y, eq3x=eq3x, eq3y=eq3y,
