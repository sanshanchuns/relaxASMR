...
def analyze_with_vlm(image_path: Path, on_progress=None) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"VLM 依赖缺失: {e}")

    # Load all potential API keys
    api_keys = {}
    key_names = ["GEMINI_API_KEY", "GEMINI_JAPAN_API_KEY", "GEMINI_USA_API_KEY"]
    
    # Check os.environ first
    for name in key_names:
        if os.environ.get(name):
            api_keys[name] = os.environ.get(name)
            
    # Check ~/.zshrc for missing keys
        ...
        with open(zshrc_path, 'r', encoding='utf-8') as f:
            for line in f:
                if line.startswith("export "):
                    ...
                    if match and match.group(1) in key_names:
                        name = match.group(1)
                        if name not in api_keys:
                            api_keys[name] = match.group(2)
    
    available_keys = [(name, api_keys[name]) for name in key_names if name in api_keys and name != "GEMINI_JAPAN_API_KEY"]
    ...
    try:
        ...
        raise RuntimeError(f"无法打开画面图片: {e}")
    ...
    for key_name, key_val in available_keys:
        try:
            ...
            for model_name in models_to_try:
                try:
                    if on_progress: on_progress(f"  -> 尝试模型: {model_name} ...")
            ...
            # Clean up any markdown blocks if VLM ignored instructions
            ...
            if on_progress: on_progress(f"  -> {err_msg}")
            else: print(f"vlm_engine: {err_msg}")
            errors.append(err_msg)
            
    # If we exhaust all available keys and none worked
    raise RuntimeError(f"所有 API Key 均测试失败: " + "; ".join(errors))
