...
def test_wsl_unc_path_uses_drive_for_drvfs_mnt(monkeypatch) -> None:
    ...
    with patch("media_paths._mnt_is_remote_fs", return_value=False):
        assert wsl_unc_path(p) == (
            "E:\\自然之声\\to_youtube\\audio\\1_rain\\booms\\test.wav"
        )


def test_wsl_unc_path_cifs_mnt_uses_wsl_localhost(monkeypatch) -> None:
    """协作机：/mnt/e 为 CIFS 时走 \\\\wsl.localhost\\...\\mnt\\e\\...，兼容 /mnt/e，不写 NAS IP。"""
    monkeypatch.setenv("WSL_DISTRO_NAME", "Ubuntu")
    monkeypatch.delenv("RELAXASMR_MEDIA_WIN_DRIVE", raising=False)
    ...
    with patch.object(media_paths.Path, "read_text", return_value=mounts):
        with patch.object(Path, "resolve", return_value=p):
            out = wsl_unc_path(p)
    assert out == (
        "\\\\wsl.localhost\\Ubuntu\\mnt\\e\\自然之声\\to_youtube\\audio\\1_rain\\booms\\BI24.湿地雨林.wav"
    )
    assert not out.startswith("E:")
    assert "192.168.3.128" not in out
